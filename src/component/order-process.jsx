...
const OrderProcessButton = () => {
  ...
  const approveOrder = async () => {
    setLoading(true);
    try {
      const response = await axios.patch(`/api/orders/${id}/approve`, { role });
  ...
  const startDeparture = async () => {
    setLoading(true);
    try {
      const response = await axios.patch(`/api/orders/${id}/start-departure`);
      setStatusOrder(response.data.statusOrder);
    } catch (error) {
      setMsg(error.response ? error.response.data.msg : error.message);
    } finally {
  ...
  const completeDeparture = async () => {
    ...
    try {
      const response = await axios.patch(
        `https://c-greenproject.org:8000/orders/${id}/complete-departure`
      );
      setStatusOrder(response.data.statusOrder);
    } catch (error) {
      setMsg(error.response ? error.response.data.msg : error.message);
  ...
  const processAtFactory = async () => {
    ...
    try {
      const response = await axios.patch(
        `https://c-greenproject.org:8000/orders/${id}/process-factory`
      );
      setStatusOrder(response.data.statusOrder);
    } catch (error) {
      setMsg(error.response ? error.response.data.msg : error.message);
    ...
    const fetchOrderStatus = async () => {
      try {
        const response = await axios.get(
          `https://c-greenproject.org:8000/orders/${id}`
        );
        setStatusOrder(response.data.statusOrder);
      } catch (error) {
        setMsg(error.response ? error.response.data.msg : error.message);
